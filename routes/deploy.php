...
Route::get('/__deploy/{action}', DeployController::class)
    ->where('action', 'status|db-status|db-create|migrate|migrate-fresh|migrate-repair|setup|clear|optimize|queue-restart|queue-work|failed-jobs|hacienda|mail-test|seed')
    ->name('deploy');
